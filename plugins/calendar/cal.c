...
typedef struct {
    ...
    calendar_w* cal;    //New calendar object
...
/* Display a new style calendar widget. */
static GtkWidget * dclock_create_calendar(DClockPlugin * dc){
    ...
    date_w* ddd= get_month_begin( date_w_add_months(get_current_day(),-0));

    dc->cal=calendar_new(ddd);

    gtk_container_add (GTK_CONTAINER (win), dc->cal->container);    
...
static GtkWidget * dclock_create_calendar_old(DClockPlugin * dc)
{
    /* Create a new window. */
    GtkWidget * win = gtk_window_new(GTK_WINDOW_TOPLEVEL);
    gint x, y;
    ...
    else
    {
        ...
        else
        {
            ...
            for(int i=0;i<7;i++){
                ...
                free_lw(dc->cal->dw_labels[i]);
            }
            
            
            for(int i=0;i<42;i++){
                
                free_dw(dc->cal->cal_grid[i]);
                
            }
            
            
            gtk_widget_destroy(dc->cal->changer->self);
            gtk_widget_destroy(dc->cal->container);
            
//            for(int i=0;i<7;i++){
//                gtk_grid_remove_column((GtkGrid*)dc->cal->self,i);
//            }
            
            //gtk_widget_destroy(dc->cal->self);
...
static GtkWidget *dclock_constructor(LXPanel *panel, config_setting_t *settings)
{
    ...
    if (config_setting_lookup_int(settings, "FontSize", &tmp_int)){
        if(tmp_int == 0){
            dc->fsize=10;
        }
        else{
    ...
    GtkWidget * hbox =gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 0); // fix by MS
...
//    PangoAttribute *pg_attr = pango_attr_size_new(12);//dc->fsize
    
    dc->clock_label = gtk_label_new(NULL);
    
//    pango_attr_list_change(pg_attr_list, pg_attr);
//    gtk_label_set_attributes(GTK_LABEL(dc->clock_label), pg_attr_list);
    
    gtk_widget_set_name(dc->clock_label, "foo");
...
static gboolean dclock_apply_configuration(gpointer user_data)
{
    ...
    config_group_set_int(dc->settings, "FontSize", dc->fsize);
    return FALSE;
}
...
static GtkWidget *dclock_configure(LXPanel *panel, GtkWidget *p)
{
    ...
    return lxpanel_generic_config_dlg(_("Digital Clock"), panel,
        ...
        _("Panel widget font size"), &dc->fsize, CONF_TYPE_INT, 
...
FM_DEFINE_MODULE(lxpanel_gtk, cal)

/* Plugin descriptor. */
LXPanelPluginInit fm_module_init_lxpanel_gtk = {
    .name = N_("Calendar and Clock widget"),
    .description = N_("Display digital clock and date"),
