...
gtk_weather_init(GtkWeather * weather)
{
  ...
  priv->hbox = gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 1); // fix by MS
  ...
  if (priv->location && priv->forecast)
    {
      ...
      gtk_widget_get_preferred_size(GTK_WIDGET(priv->hbox),NULL,&req);// fix by MS
...
gtk_weather_change_location(GtkWidget * widget, GdkEventButton * event)
{
  ...
  GtkWidget * entry_hbox = gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 10); // fix by MS
  ...
  GtkWidget * label_hbox = gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 10); // fix by MS
...
gtk_weather_create_preferences_dialog(GtkWidget * widget)
{
  ...
  GtkWidget * location_hbox = gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 1); // fix by MS
  ...
  GtkWidget * button_hbox = gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 10); // fix by MS
  ...
  GtkWidget * auto_hbox = gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 2); // fix by MS
  ...
  if (location && forecast)
    {
      ...
      GtkWidget * everything_hbox = gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 5); // fix by MS
       ...
       */
      GtkWidget * icon_image = gtk_image_new_from_stock("image-missing",
                                                        GTK_ICON_SIZE_MENU);
      ...
      gtk_widget_get_preferred_size(dialog,NULL,&req);// fix by MS
