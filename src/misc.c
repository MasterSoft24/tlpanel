...
static void _gtk_image_set_from_file_scaled(GtkWidget * img, ImgData * data)
{
    ...
    else
    {
        ...
        gtk_image_set_from_icon_name(GTK_IMAGE(img), "image-missing", GTK_ICON_SIZE_BUTTON); // fix by MS
    }
}
...
get_button_spacing(GtkRequisition *req, GtkContainer *parent, gchar *name)
{
    ...
    gtk_widget_get_preferred_size(b,NULL,req);// fix by MS

    gtk_widget_destroy(b);
    RET();
}

// new variant for GtkRGBA colors using 
guint32 _gcolor2rgb24(GdkRGBA *color)// fix by MS
{
    guint32 i;
    //gdouble d=0.00390625;
    guint32 ir,ig,ib;
...
//    i = (((guint32) (color->red/d)) * 0xFF / 0xFFFF) & 0xFF;
//    i <<= 8;
//    i |= (((guint32) (color->green/d)) * 0xFF / 0xFFFF) & 0xFF;
//    i <<= 8;
//    i |= (((guint32) (color->blue/d)) * 0xFF / 0xFFFF) & 0xFF;
    

    ir = ((guint32) (color->red * 255));
    ig = ((guint32) (color->green * 255));
    ib = ((guint32) (color->blue * 255));
    ir <<= 16;
    ig <<= 8;
...
// convert 24 bit color to GdkRgba format back
void _rgb24_to_gcolor(guint32 color,GdkRGBA* gcolor){// fix by MS
    ...
    gcolor->red = (gdouble)((gdouble)(r) * d);
    gcolor->green= (gdouble)(g) * d;
    gcolor->blue= (gdouble)(b) * d;
}
...
//    i = (((gint16) (color->red/d)) * 0xFF / 0xFFFF) & 0xFF;
//    i <<= 8;
//    i |= (((gint16) (color->green/d)) * 0xFF / 0xFFFF) & 0xFF;
//    i <<= 8;
//    i |= (((gint16) (color->blue/d)) * 0xFF / 0xFFFF) & 0xFF;
                                          ...
                                          gulong highlight_color, const gchar *label)
{
    ...
    gtk_widget_set_margin_top(image,0); // fix by MS
    ...
    gtk_widget_set_halign(image,GTK_ALIGN_CENTER);// fix by MS
    ...
    else
    {
        ...
        inner = gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 0); // fix by MS
