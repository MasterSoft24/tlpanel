...
static void lxpanel_size_request(GtkWidget *widget, GtkRequisition *req)
{
    ...
    if (!p->visible)
        ...
        gtk_widget_get_preferred_size(p->box,NULL,req);// fix by MS
...
static void lxpanel_init(PanelToplevel *self)
{
    ...
    gdk_rgba_parse( &p->gtintcolor,"white"); // fix by MS
    p->tintcolor = 0;//_gcolor2rgb24(&p->gtintcolor);
    p->usefontcolor = 0;
...
void _panel_determine_background_pixmap(LXPanel * panel)
{
    ...
    else if (p->surface == NULL)
    {
        ...
        else
        {
            ...
            gdk_cairo_set_source_rgba(cr, &p->gtintcolor);// fix by MS
                           ...
                           gboolean custom_color)
{
    ...
    gchar *stringPlainText; // fix by MS
    PangoAttrList  *attrList;
    
    pango_parse_markup(formatted_text,-1,0,&attrList, &stringPlainText, NULL, NULL);
...
//    }
    ...
    if (config_setting_lookup_string(cfg, "tintcolor", &str)) //fix by MS
    {
        if (!gdk_rgba_parse ( &p->gtintcolor, str))
            gdk_rgba_parse ( &p->gtintcolor,"white");
        p->tintcolor = _gcolor2rgb24(&p->gtintcolor);
            DBG("tintcolor=%x\n", p->tintcolor);
    }
...
//    }
    ...
    if (config_setting_lookup_string(cfg, "fontcolor", &str))// fix by MS
    {
        if (!gdk_rgba_parse ( &p->gfontcolor,str))
            gdk_color_parse ( &p->gfontcolor,"black");
        p->fontcolor = gcolor2rgb24(&p->gfontcolor);
            DBG("fontcolor=%x\n", p->fontcolor);
    }    
...
//    return gtk_vbox_new(homogeneous, spacing);
    ...
    if (panel->priv->orientation == GTK_ORIENTATION_HORIZONTAL){
        return gtk_box_new(GTK_ORIENTATION_HORIZONTAL, spacing); // fix by MS
    }
    else{
        return gtk_box_new(GTK_ORIENTATION_VERTICAL, spacing); // fix by MS
    }
