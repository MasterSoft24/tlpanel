...
#define UPDATE_GLOBAL_COLOR(panel,name,val) do { \
    ...
    if (_s) { \
        ...
        _nc = gdk_rgba_to_string (&val); \
        config_setting_set_string(_s,_nc); } } while(0)
...
on_tint_color_set( GtkColorButton* clr,  Panel* p )
{
    ...
    gtk_color_chooser_get_rgba( clr, &p->gtintcolor );// fix by MS
    p->tintcolor = _gcolor2rgb24(&p->gtintcolor);
    p->alpha = gtk_color_button_get_alpha( clr ) / alpha_scale_factor;
    panel_update_background( p );
    UPDATE_GLOBAL_COLOR(p, "tintcolor", p->gtintcolor);
...
void panel_configure( LXPanel* panel, int sel_page )
{
    ...
    */
    ...
    gtk_color_button_set_rgba(GTK_COLOR_BUTTON(w), &p->gtintcolor );
    //gtk_color_button_set_color(GTK_COLOR_BUTTON(w), _gcolor2rgb24(&p->gtintcolor) );
    ...
    gtk_color_button_set_rgba(GTK_COLOR_BUTTON(w), &p->gfontcolor );//fix by MS
    ...
    while( name )
    {
        ...
        if( entry )
        {
            ...
            else
            {
                ...
                GtkWidget* hbox = gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 2); // fix by MS
